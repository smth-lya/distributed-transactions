...
{
    private readonly Dictionary<Guid, OrderCreatedEvent> _orderCreatedEvents = new ();
    ...
    // Какая проблема?
    // Сейчас оркестратор, как и другие сервисные workers конфигурируют соединение внутри себя
    //
    //
    ...
    public override async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        await SubscribeOnServicesEvents(cancellationToken);
    ...
    private async Task SubscribeOnServicesEvents(CancellationToken cancellationToken = default)
    {
        var tasks = new List<Task>
        {
            SubscribeAsync<OrderCreatedEvent>("orchestrator.evt.q", HandleOrderCreated, cancellationToken: cancellationToken),
            SubscribeAsync<InventoryReservedEvent>("orchestrator.evt.q", HandleItemsReserved, cancellationToken),
            SubscribeAsync<PaymentCompletedEvent>("orchestrator.evt.q", HandlePaymentCompleted, cancellationToken),
        };
    ...
    private async Task PublishAsync<T>(T message, string exchange = "", string routingKey = "", CancellationToken cancellationToken = default) where T : Message
    {
    ...
    private async Task HandleOrderCreated(OrderCreatedEvent arg)
    {
        _orderCreatedEvents[arg.CorrelationId] = arg;
        
        var command = new ReserveInventoryCommand(
            Items: [new InventoryItem(arg.ProductId, arg.Quantity)])
        {
            CorrelationId = arg.CorrelationId
        };
     
        _logger.LogInformation("HandleOrderCreated");
    ...
    private async Task HandleItemsReserved(InventoryReservedEvent arg)
    {
        _orderCreatedEvents.TryGetValue(arg.CorrelationId, out var orderCreatedEvent);

        var command = new ProcessPaymentCommand(orderCreatedEvent.TotalPrice, "USD", 0)
        {
            CorrelationId = arg.CorrelationId
        };
        
        _logger.LogInformation("HandleItemsReserved");
    ...
    private async Task HandlePaymentCompleted(PaymentCompletedEvent arg)
    {
        ...
        _logger.LogInformation("HandlePaymentCompleted");
