...
public class PaymentWorker : BackgroundService
{
    ...
    private ILogger<PaymentWorker> _logger;
    ...
    private async Task ConfigureRabbitMq(CancellationToken cancellationToken = default)
    {
        ...
        await _channel.QueueDeclareAsync($"payment.cmd.q", durable: true, exclusive: false, autoDelete: false,
    ...
    private async Task SubscribeOnSagaCommands(CancellationToken cancellationToken)
    {
        ...
        {
            SubscribeAsync<ProcessPaymentCommand>("payment.cmd.q", HandlePayment, cancellationToken)
        };
        
        await Task.WhenAll(tasks);
    }

    private async Task HandlePayment(ProcessPaymentCommand arg)
    {
        var message = new PaymentCompletedEvent(Guid.NewGuid().ToString(), arg.Amount);

        _logger.LogInformation("HandlePayment");
        
        await PublishAsync(message, exchange: "saga.fanout.evt", routingKey: string.Empty);
    }
